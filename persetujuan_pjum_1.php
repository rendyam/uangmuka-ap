...
if (isset($_POST['btnApprove']) and $_SERVER['REQUEST_METHOD'] == "POST") {
    try {
        ...
        if ($txtAct == 1) {
            ...
            //                                    $mail->IsHTML(true);
            //                                    $mail->Subject = $txtKode." - Persetujuan Pertanggungjawaban Uang Muka";
            //                                    $mail->Body    = "Untuk melakukan persetujuan pertanggungjawaban uang muka klik alamat di bawah ini :<br><a href='$externalLink/uangmuka/persetujuan_pjum.php?id=".base64_encode($txtKode)."&param=".base64_encode($row[2])."&u=".base64_encode($row[0])."&act=3'>Link Internal Sistem Uang Muka</a>"."<br><br><a href='$externalLink/uangmuka/persetujuan_pjum.php?id=".base64_encode($txtKode)."&param=".  base64_encode($row[2])."&u=".  base64_encode($row[0])."&act=3'>Link External Sistem Uang Muka</a>";
            //                                    $mail->Send();
            //
            //                                    //$strMessage = "<div class='alert alert-success'><strong>Data berhasil disimpan</strong></div>";
            //                                    header('location:pertanggungjawaban_overview.php');
