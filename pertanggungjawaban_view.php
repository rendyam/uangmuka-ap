...
if (isset($_POST['btnResend']) and $_SERVER['REQUEST_METHOD'] == "POST") {
    try {
        ...
        $query = "select status_ref from t_pjum_status_tab where t_pjum_id = '" . $txtKode . "' and pjum_status is null limit 1";
        //echo $query;
        $result = $db->prepare($query);
        $result->execute();
        ...
        if ($txtAct == 1) {
            ...
            if ($num > 0) {
                while ($row = $result->fetch(PDO::FETCH_NUM)) {
                    ...
                    $mail->IsHTML(true);
                    $mail->Subject = $txtKode . " - (re-send) Persetujuan Pertanggungjawaban Uang Muka";
            ...
            $query = "select pj_user_penyetuju_id, pj_user_penyetuju_email, tgl_pjum from t_um_rpt where t_pjum_id = '" . $txtKode . "'";
            ...
            $query = "select pj_pengadaan_disiapkan_id, pj_pengadaan_disiapkan_email, tgl_pjum from t_um_rpt where t_pjum_id = '" . $txtKode . "'";
            ...
            $query = "select pj_pengadaan_disetujui_id, pj_pengadaan_disetujui_email, tgl_pjum from t_um_rpt where t_pjum_id = '" . $txtKode . "'";
