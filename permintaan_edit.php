...
if (isset($_POST['btnSimpan']) and $_SERVER['REQUEST_METHOD'] == "POST") {
    try {
        ...
        if ($txtKeperluan == "" || $txtPeriode == "" || $cmbUser1 == "" || $cmbUser2 == "") {
            $strMessage = "<div class='alert alert-error'><strong>Kolom dengan tanda bintang (*) wajib diisi</strong></div>";
        } else {
            if ($txtLampiran == '') {
                $db->beginTransaction();

                $sqlQuery = $db->prepare("update t_um_tab set keperluan = :keperluan, periode = :periode, user_peminta = :user_peminta, user_penyetuju = :user_penyetuju, pengadaan_disiapkan = :pengadaan_disiapkan, pengadaan_disetujui = :pengadaan_disetujui where t_um_id = :t_um_id");
                ...
                if (strlen($name)) {
                    ...
                    if (move_uploaded_file($tmp, $path . $actual_image_name)) {
                        ...
                        $sqlQuery = $db->prepare("update t_um_tab set keperluan = :keperluan, periode = :periode, user_peminta = :user_peminta, user_penyetuju = :user_penyetuju, pengadaan_disiapkan = :pengadaan_disiapkan, pengadaan_disetujui = :pengadaan_disetujui, lampiran_um = :lampiran_um where t_um_id = :t_um_id");
            ...
            while ($row = $result->fetch(PDO::FETCH_NUM)) {
                ...
                if ($sqlQuery->rowCount() > 0) {
                    ...
                    if ($sqlQuery->rowCount() > 0) {
                        ...
                        $mail->IsHTML(true);
                        $mail->Subject = $txtKode . " - Persetujuan Permintaan Uang Muka No : ";
...
try {
    ...
    $query = "SELECT count(t_um_id) FROM lock_aging_rpt where user_peminta = " . $cmbUser1;
    ...
    if ($num > 0) {
        while ($row = $result->fetch(PDO::FETCH_NUM)) {
            if ($row[0] >= 1) {
                $flagLock = 1;
                $strLockPeminta = "<div class='alert alert-danger alert-fill alert-close alert-dismissible fade show' role='alert'>User Peminta terkena Lock Aging, cek UM yang belum dibuat PJUM dan PJUM belum Approved di halaman Home</div>";
            }
        }
    }
} catch (Exception $e) {
            ...
            <div class="box-typical box-typical-padding">
                ...
                <form id="frmPermintaan" action="" method="post" enctype="multipart/form-data">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <p class="form-control-static"><input type="text" class="form-control" name="txtNilaiUM" id="txtNilaiUM" value="<?PHP echo $txtNilaiUM; ?>" disabled></p>
                        </div>
