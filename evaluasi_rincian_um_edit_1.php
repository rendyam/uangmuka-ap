    ...
    if (isset($_POST['btnSimpan']) and $_SERVER['REQUEST_METHOD'] == "POST") {
        try {
            ...
            if($txtQty=="" || $txtHarga==""){
                ...
                $sqlQuery = $db->prepare("update t_um_detail_tab set qty_evaluasi = :qty_pengajuan, harga_evaluasi = :harga_pengajuan where t_um_detail_id = :t_um_detail_id and t_um_id = :t_um_id");
                ...
                if($sqlQuery->rowCount() > 0){
                    if($txtHarga < $txtHargaAwal){
                        ...
                        $mail->IsHTML(true);
                        $mail->Subject = "Evaluasi Permintaan Uang Muka No : ".$txtKode;
                        $mail->Body    = "Permintaan uang muka anda nomor : ".$txtKode." dievaluasi oleh keuangan.";
                        $mail->Send();
                    }
                    
                    header('location:persetujuan_um_1.php?id='.base64_encode($txtKode).'&param='.$_GET['param'].'&u='.$_GET['u'].'&act='.$_GET['act']);
                ...
                <div class="box-typical box-typical-padding">
                    ...
                    <form id="frmRincian" action="" method="post" enctype="multipart/form-data">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <p class="form-control-static"><?PHP echo $txtRincian;?></p>
                            </div>
                            <label class="col-sm-2 form-control-label">Qty Evaluasi *</label>
                            <div class="col-sm-10">
                                <p class="form-control-static"><input type="text" class="form-control" name="txtQty" id="txtQty" value="<?PHP echo $txtQty;?>"></p>
                            </div>
                            <label class="col-sm-2 form-control-label">Harga Evaluasi *</label>
                            <div class="col-sm-10">
