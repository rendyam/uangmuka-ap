    ...
    try {
        ...
        if ($txtAct == 1) {
            ...
            if ($num > 0) {
                while ($row = $result->fetch(PDO::FETCH_NUM)) {
                    ...
                    if ($sqlQuery->rowCount() > 0) {
                        ...
                        $mail->IsHTML(true);
                        $mail->Subject = $txtKode . " - Persetujuan Permintaan Uang Muka";
            ...
            if ($num > 0) {
                ...
                if ($sqlQuery->rowCount() > 0) {
                    //$strMessage = "<div class='alert alert-success'><strong>Data berhasil disimpan</strong></div>";
                    $query = "select id, email, created_at from position_user_rpt where um_pencairan = 1";
                    ...
                    if ($num > 0) {
                        while ($row = $result->fetch(PDO::FETCH_NUM)) {
                            ...
                            $mail->FromName = $from_name;
                            $mail->IsHTML(true);
                            $mail->Subject = $txtKode . " - Pencairan Permintaan Uang Muka";
                            $mail->Body    = "Untuk melakukan pencairan permintaan uang muka klik alamat di bawah ini :<br><a href='$internalLink/uangmuka/evaluasi_um.php?id=" . base64_encode($txtKode) . "&param=" . base64_encode($row[2]) . "&u=" . base64_encode($row[0]) . "'>Link Internal Sistem Uang Muka</a>" . "<br><br><a href='$externalLink/uangmuka/evaluasi_um.php?id=" . base64_encode($txtKode) . "&param=" .  base64_encode($row[2]) . "&u=" .  base64_encode($row[0]) . "'>Link External Sistem Uang Muka</a>";
                            $mail->Send();
...
if (isset($_POST['btnReject']) and $_SERVER['REQUEST_METHOD'] == "POST") {
    try {
        ...
        if ($txtAlasan == "") {
            ...
            $sqlQuery = $db->prepare("update t_um_tab set status_um = :status_um, note_reject = :note_reject where t_um_id = :t_um_id");
            ...
            if ($sqlQuery->rowCount() > 0) {
                ...
                $mail->FromName = $from_name;
                $mail->IsHTML(true);
                $mail->Subject = $txtKode . " Ditolak - Permintaan Uang Muka No : ";
                $mail->Body    = "Permintaan uang muka anda nomor : " . $txtKode . " ditolak.";
                $mail->Send();
            ...
            <div class="box-typical box-typical-padding">
                <div class="form-group row">
                    ...
                    if ($approvalStatus == "") {
                        ...
                        if ($txtAct == 3 || $txtAct == 4 || $txtAct == 5 || $txtAct == 6) {
                            echo "<div class='col-sm-1'>
                                                <a class='btn btn-success' href='evaluasi_rincian_um_1.php?id=" . $_GET['id'] . "&param=" . $_GET['param'] . "&u=" . $_GET['u'] . "&act=" . $_GET['act'] . "'>
