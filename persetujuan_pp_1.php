...
if (isset($_POST['btnApprove']) and $_SERVER['REQUEST_METHOD'] == "POST") {
    try {
        ...
        if ($txtAct == 1) {
            $query = "select user_penyetuju_id, user_penyetuju_email, tgl_um from t_um_rpt where t_um_id = (select t_um_id from t_dispensasi_tab where t_dispensasi_id = '" . $txtKode . "') and tgl_um = (SELECT yy.tgl_um FROM t_um_tab yy WHERE yy.t_um_id = (SELECT xx.t_um_id FROM t_dispensasi_tab xx WHERE xx.t_dispensasi_id = '" . $txtKode . "' AND xx.tgl_dispensasi = '" . $txtParam . "'))";
            ...
            $query = "select keuangan_penyetuju_1_id, keuangan_penyetuju_1_email, tgl_um from t_um_rpt where t_um_id = (select t_um_id from t_dispensasi_tab where t_dispensasi_id = '" . $txtKode . "') and tgl_um = (SELECT yy.tgl_um FROM t_um_tab yy WHERE yy.t_um_id = (SELECT xx.t_um_id FROM t_dispensasi_tab xx WHERE xx.t_dispensasi_id = '" . $txtKode . "' AND xx.tgl_dispensasi = '" . $txtParam . "'))";
